...
namespace EntityModel.Test
{
    ...
    public class SqlQueryTests
    {
        ...
        public void Query_builder_contains_where_with_date_start_and_end()
        {
            ...
            queryString = _sCAuditService.QueryBuilder();
            Assert.IsTrue(queryString.Contains("WHERE "), "Query 02 string should not contain WHERE clause");
            Assert.IsTrue(queryString.Contains(">= CONVERT(DATETIME, '"+ startDateTimeString +""), "Query 02 string does not contain correct start date: " + queryString);
            ...
            queryString = _sCAuditService.QueryBuilder();
            Assert.IsTrue(queryString.Contains("<= CONVERT(DATETIME, '" + endDateTime.Date + ""), "Query 02 string does not contain correct end date: " + queryString);

            _sCAuditService.StartDate = startDateTime;
            _sCAuditService.EndDate = endDateTime;
            queryString = _sCAuditService.QueryBuilder();
            Assert.IsTrue(queryString.Contains("<= CONVERT(DATETIME, '" + endDateTime.AddDays(1).Date + ""), "Query 03 string does not contain correct end date: " + queryString);
        }
        ...
        public void Query_builder_contains_where_with_date_start_and_Range()
        {
            ...
            queryString = _sCAuditService.QueryBuilder();
            Assert.IsTrue(queryString.Contains("WHERE "), "Query 01 string should contain WHERE clause");
            Assert.IsTrue(queryString.Contains(">= CONVERT(DATETIME, '" + startDateTimeString + ""), "Query 01 string should not contain correct start date: " + queryString);
            Assert.IsTrue(queryString.Contains("<= CONVERT(DATETIME, '" + endDateTimeString + ""), "Query 01 string should not contain correct end date: " + queryString);

            endDateTimeString = "30/06/2017";
            _sCAuditService.StartDate = startDateTime;
            _sCAuditService.DateRange = 20;
            queryString = _sCAuditService.QueryBuilder();
            Assert.IsTrue(queryString.Contains("WHERE "), "Query 02 string should contain WHERE clause");
            Assert.IsTrue(queryString.Contains(">= CONVERT(DATETIME, '" + startDateTimeString + ""), "Query 02 string should not contain correct start date: " + queryString);
            Assert.IsTrue(queryString.Contains("<= CONVERT(DATETIME, '" + endDateTimeString + ""), "Query 02 string should not contain correct end date: " + queryString);

            endDateTimeString = "10/07/2017";
            _sCAuditService.StartDate = startDateTime;
            _sCAuditService.DateRange = 30;
            queryString = _sCAuditService.QueryBuilder();
            Assert.IsTrue(queryString.Contains("WHERE "), "Query 03 string should contain WHERE clause");
            Assert.IsTrue(queryString.Contains(">= CONVERT(DATETIME, '" + startDateTimeString + ""), "Query 03 string should not contain correct start date: " + queryString);
            Assert.IsTrue(queryString.Contains("<= CONVERT(DATETIME, '" + endDateTimeString + ""), "Query 03 string should not contain correct end date: " + queryString);
        }
        ...
        [TestMethod]
        public void Query_builder_Where_PartType_is_R()
        {
            var returnList = new List<SCAudit>();
            var startDateTimeString = "";
            ...
            _sCAuditService.DateRange = 1;
            _sCAuditService.PageCount = 100;
            _sCAuditService.WhereExpression = "WHERE [Part_Type] = 'R'";
            returnList = _sCAuditService.GetAll();

            Assert.IsNotNull(returnList, "ReturnList returned null");
            Assert.IsTrue(returnList.Any(), "ReturnList didn't any records : " + returnList.Count.ToString());
            foreach (var item in returnList)
            {
                Assert.IsTrue(item.Audit_Last_Update.Date >= startDateTime.Date, "ReturnList didn't return 10 records : " + returnList.First().Audit_Last_Update.ToString());
                Assert.IsTrue(item.Audit_Last_Update.Date < startDateTime.AddDays(1).Date, "ReturnList didn't return 10 records : " + returnList.Last().Audit_Last_Update.ToString());
                Assert.IsTrue(item.Audit_Part_Type.Equals("R"), "Item where R wasn't returned : " + item.Audit_Part_Type);
            }
        }
    }
}
